...
import { useUnifiedCreativeCharactersSimplified } from '../hooks/useUnifiedCreativeCharactersSimplified';
...
const UnifiedCreativeCharacterLibrarySimplified = () => {
  ...
  const { userCharacters, publicCharacters, filteredCharacters } = useMemo(() => {
    ...
    const userOwned = filtered.filter(char => user && char.user_id === user.id);
    const publicOwned = filtered.filter(char => !user || char.user_id !== user.id);

    return {
      userCharacters: userOwned,
      publicCharacters: publicOwned,
      filteredCharacters: filtered
    };
  }, [characters, searchQuery, user]);
