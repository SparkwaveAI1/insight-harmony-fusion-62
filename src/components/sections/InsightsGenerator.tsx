...
import { FileJson, Link, Share2 } from "lucide-react";
...
const InsightsGenerator = () => {
  ...
  const generateInsights = async () => {
    ...
    try {
      ...
      const researchQuery: ResearchQuery = {
        ...
        keywords: query.split(" ").filter(word => word.length > 5) // Simple keyword extraction
      };
      
      // Use the real data service
  ...
  return (
    <section id="insights-generator" className="py-16 bg-gradient-to-b from-background to-muted/20">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row gap-4 mb-8">
            <Input
              ...
              className="flex-grow"
            />
            <Button onClick={generateInsights} disabled={isLoading} className="shrink-0">
  ...
  <Card className="border-dashed border-2 bg-transparent">
    <CardContent className="pt-6 text-center py-16">
      ...
      <p className="text-xs text-muted-foreground/70">
        Try queries like "Web3 adoption barriers" or "AI regulation sentiment"
      </p>
    </CardContent>
  </Card>
);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-2">
        <p className="text-sm text-muted-foreground">
          Analysis generated at {new Date(results.reportGeneratedAt).toLocaleTimeString()}
        </p>
