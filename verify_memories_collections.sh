...
P1=$(curl -sfS "${HDR_AUTH[@]}" "$MEM_URL?persona_id=$PERSONA_ID&limit=10") || fail "baseline request failed"
...
if [[ -n "${CUR1:-}" ]]; then
  ...
  P2=$(curl -sfS "${HDR_AUTH[@]}" "$MEM_URL?persona_id=$PERSONA_ID&limit=10&cursor=$enc_cur") || fail "page2 request failed"
...
PF=$(curl -sfS "${HDR_AUTH[@]}" "$MEM_URL?persona_id=$PERSONA_ID&type=facts&limit=10") || fail "type filter failed"
...
PT=$(curl -sfS "${HDR_AUTH[@]}" "$MEM_URL?persona_id=$PERSONA_ID&tag=news&limit=10") || fail "tag filter failed"
...
CL=$(curl -sfS "${HDR_AUTH[@]}" "$COLL_URL?persona_id=$PERSONA_ID") || fail "collections failed"
...
TAMP=$(curl -s -o /dev/null -w "%{http_code}" "${HDR_AUTH[@]}" "$MEM_URL?persona_id=$PERSONA_ID&limit=10&cursor=$(printf bogus\|cursor | jq -sRr @uri)")
