...
export const useMessageOperations = ({
  ...
  const handleUserSendMessage = async (
    ...
    console.log(`User message to ${targetPersona}: ${userInput}`);
    console.log(`Persona IDs - A: ${personaAId}, B: ${personaBId}`);
    ...
    if (!autoChatActive) {
      try {
        const targetPersonaId = targetPersona === 'personaA' ? personaAId : personaBId;
        console.log(`Generating response for ${targetPersona} (ID: ${targetPersonaId})`);
  ...
  const handleUserSendMessageToTarget = async (
    ...
    console.log(`Direct message to ${targetPersona}: ${messageText}`);
    console.log(`Persona IDs - A: ${personaAId}, B: ${personaBId}`);
