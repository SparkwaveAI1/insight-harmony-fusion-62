...
const V4PersonaCard: React.FC<V4PersonaCardProps> = ({ 
  persona, 
  onVisibilityChange, 
  onDelete,
  ...
  const handleVisibilityChange = async () => {
    ...
    try {
      // TODO: Implement V4 persona visibility update
      setIsPublic(newVisibility);
      if (onVisibilityChange) {
        onVisibilityChange(persona.persona_id, newVisibility);
      }
    } catch (error) {
      console.error("Error updating V4 persona visibility:", error);
    }
  };
  ...
  // Get V4 demographic data with proper typing
  ...
  const backgroundDescription = demographics?.background_description || 'No description available';

  // Format creation date
  const creationDate = persona.created_at 
    ? new Date(persona.created_at).toLocaleDateString() 
    : 'Unknown';

  // Get creation stage badge
  const getCreationStageBadge = () => {
    const stage = persona.creation_stage || 'not_started';
    const completed = persona.creation_completed;
    
    if (completed) {
      return (
        <Badge variant="default" className="text-xs px-2 py-0.5 bg-green-50 text-green-700 dark:bg-green-950/20 dark:text-green-400">
          Complete
        </Badge>
      );
    }
    ...
    switch (stage) {
      case 'detailed_traits':
      case 'summary_generation':
        return (
          <Badge variant="secondary" className="text-xs px-2 py-0.5 bg-blue-50 text-blue-700 dark:bg-blue-950/20 dark:text-blue-400">
            In Progress
          </Badge>
        );
      case 'not_started':
      ...
      <CardContent onClick={handleViewDetails} className="cursor-pointer pt-0 pb-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-foreground line-clamp-3 leading-relaxed">
            {backgroundDescription.length > 150 ? `${backgroundDescription.substring(0, 150)}...` : backgroundDescription}
          </p>
        </div>
      </CardContent>
