...
const CreativeCharactersDashboard = () => {
  const { data: allCharacters = [], isLoading, error } = useCharacters();
  const [searchTerm, setSearchTerm] = React.useState('');

  // Filter for creative characters only
  const creativeCharacters = React.useMemo(() => {
    return allCharacters.filter(character => character.character_type === 'creative');
  }, [allCharacters]);

  const filteredCharacters = React.useMemo(() => {
    return creativeCharacters.filter(character => {
      const matchesSearch = character.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           character.metadata?.description?.toLowerCase().includes(searchTerm.toLowerCase());
      return matchesSearch;
    });
  }, [creativeCharacters, searchTerm]);
      ...
      <Section>
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold">Creative Characters</h1>
              <p className="text-muted-foreground">
                Manage original characters from any genre or universe
              </p>
            </div>
          </div>
        ...
        {filteredCharacters.length === 0 ? (
          <Card className="text-center py-12">
            ...
            <h2 className="text-xl font-semibold mb-2">No Creative Characters Found</h2>
            <p className="text-muted-foreground mb-6">
              {creativeCharacters.length === 0 
                ? "You haven't created any creative characters yet." 
                : "No creative characters match your search criteria."
              }
            </p>
            <Button asChild>
              <Link to="/characters/create/creative">
...
export default CreativeCharactersDashboard;
