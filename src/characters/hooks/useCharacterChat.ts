...
import { breakIntoMultipleMessages } from '@/components/persona-chat/utils/chatMessageUtils';
import { sendMessageToCharacter } from '../services/characterChatService';
  ...
  useEffect(() => {
    if (activeCharacter && messages.length === 0) {
      setMessages([
        {
          role: 'assistant',
          content: `Hello! I'm ${activeCharacter.name}.`,
          timestamp: new Date(),
        },
        {
          role: 'assistant',
