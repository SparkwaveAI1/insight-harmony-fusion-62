...
begin
  ...
  if not exists (
    ...
    execute $g$
      ...
      generated always as ((full_profile->'identity'->>'education_level')) stored
    $g$;
  end if;
  ...
  if not exists (
    ...
    execute $g$
      ...
      generated always as ((full_profile->'identity'->>'income_bracket')) stored
    $g$;
  end if;
  ...
  if not exists (
    ...
    execute $g$
      ...
      generated always as (((full_profile->'cognitive_profile'->>'thought_coherence')::numeric)) stored
    $g$;
  end if;
end$$;
...
set full_profile = jsonb_set(
            ...
            coalesce(to_jsonb(full_profile->'knowledge_profile'->>'education_level'),
                     to_jsonb('unspecified'::text)),
            true
          )
where (full_profile->'identity'->>'education_level') is null
   or (full_profile->'identity'->>'education_level') = '';
...
where (full_profile->'identity'->>'income_bracket') is null;
...
where full_profile ? 'truth_honesty_profile' = false;
...
set full_profile = jsonb_set(
            ...
            jsonb_build_object(
              'cognitive', jsonb_build_object(
                'status_quo', 0.5,
                'loss_aversion', 0.5,
                'confirmation', coalesce(nullif((full_profile->'inhibitor_profile'->>'confirmation_bias')::text,'' )::numeric, 0.5),
                'anchoring', 0.5,
                'availability', 0.5,
                'optimism', 0.5,
                'sunk_cost', 0.5,
...
where full_profile ? 'bias_profile' = false;
...
where full_profile ? 'cognitive_profile' = false;
...
set full_profile = jsonb_set(
            ...
            to_jsonb(
              trim(
                both ' '
                from (
                  coalesce(full_profile->>'attitude_narrative','') || 
                  case when full_profile->'identity_salience'->>'cultural_background' is not null
                       then ' ' || (full_profile->'identity_salience'->>'cultural_background')
                       else '' end
                )
              )
            ),
...
where full_profile ? 'identity_salience';
...
with c as (
  select persona_id,
         full_profile,
         (full_profile->'contradictions'->'primary_tension') as pt
  from v4_personas
  where full_profile ? 'contradictions'
)
...
set full_profile = (
  ...
  jsonb_set(
    ...
    coalesce(
      (p.full_profile#>'{motivation_profile,want_vs_should_tension,major_conflicts}'),
      '[]'::jsonb
    ) || coalesce(
      jsonb_build_object(
...
from c
where p.persona_id = c.persona_id;
