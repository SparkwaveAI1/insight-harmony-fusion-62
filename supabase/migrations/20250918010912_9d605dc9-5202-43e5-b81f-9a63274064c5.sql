...
-- select count(*) from persona_v4;
-- create table persona_v4_backup as table persona_v4;
...
begin
  ...
  if not exists (
    ...
    where table_name = 'persona_v4' and column_name = 'education_level'
  ) then
    execute $g$
      alter table persona_v4
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_name = 'persona_v4' and column_name = 'income_bracket'
  ) then
    execute $g$
      alter table persona_v4
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_name = 'persona_v4' and column_name = 'thought_coherence'
  ) then
    execute $g$
      alter table persona_v4
