...
interface SaveResearchConversationModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  messages: { role: "user" | "assistant"; content: string; responding_persona_id?: string }[];
  personaIds: string[];
  defaultTitle?: string;
  onSaved: (conversationId: string, projectId: string) => void;
}

const SaveResearchConversationModal = ({
  open,
  onOpenChange,
  messages,
  personaIds,
  defaultTitle = "",
  onSaved
}: SaveResearchConversationModalProps) => {
  const [title, setTitle] = useState(defaultTitle || `Research Session - ${new Date().toLocaleDateString()}`);
  ...
  const handleSaveConversation = async () => {
    ...
    try {
      // Create the conversation with persona IDs
      ...
      if (conversation) {
        // Save the messages with proper persona_id mapping
        const formattedMessages = messages.map(message => ({
          role: message.role,
          content: message.content,
          persona_id: message.responding_persona_id || null
        }));
        
        const saved = await saveConversationMessages(conversation.id, formattedMessages);
        
        if (saved) {
          toast.success("Research conversation saved successfully");
          onSaved(conversation.id, selectedProjectId);
          onOpenChange(false);
        }
      }
    } catch (error) {
      console.error("Error saving research conversation:", error);
      toast.error("Failed to save research conversation");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Save this research session to review later or share with your team.
          </DialogDescription>
        </DialogHeader>
        ...
        {isCreatingProject ? (
          ...
          <div className="space-y-4 pt-4">
            <div className="space-y-2">
              ...
              <Input
                id="title"
                placeholder="Enter a title for this research session"
                value={title}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="tags"
                placeholder="research, qualitative, user-insights"
                value={tags}
...
export default SaveResearchConversationModal;
