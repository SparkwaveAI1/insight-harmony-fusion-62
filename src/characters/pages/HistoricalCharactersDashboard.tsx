...
const HistoricalCharactersDashboard = () => {
  const { data: allCharacters = [], isLoading, error } = useCharacters();
  const [searchTerm, setSearchTerm] = React.useState('');

  // Filter for historical characters only
  const historicalCharacters = React.useMemo(() => {
    return allCharacters.filter(character => character.character_type === 'historical');
  }, [allCharacters]);

  const filteredCharacters = React.useMemo(() => {
    return historicalCharacters.filter(character => {
      const matchesSearch = character.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           character.metadata?.description?.toLowerCase().includes(searchTerm.toLowerCase());
      return matchesSearch;
    });
  }, [historicalCharacters, searchTerm]);
      ...
      <Section>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Manage characters based on real historical figures
              </p>
            </div>
          </div>
        ...
        {filteredCharacters.length === 0 ? (
          <Card className="text-center py-12">
            ...
            <Button asChild>
              <Link to="/characters/create/historical">
                Create Your First Historical Character
              </Link>
            </Button>
          </Card>
        ) : (
...
export default HistoricalCharactersDashboard;
