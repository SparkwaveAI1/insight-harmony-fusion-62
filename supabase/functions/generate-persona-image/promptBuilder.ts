...
function buildV4ImagePrompt(personaData: any): string {
  ...
  let physicalDescription = conversationSummary.physical_description || personaData.physical_description;

  if (!physicalDescription) {
    console.error("No physical description found in persona data");
    throw new Error("Physical description is required for image generation");
  }
  ...
  // Use the physical description directly - it already contains all necessary details
  // (gender, ethnicity, age, body type, clothing, etc.)
  const prompt = `Film still photograph of ${physicalDescription}. Shot in 2024, sigma 85mm f/1.4, natural lighting, authentic portrait.`;
