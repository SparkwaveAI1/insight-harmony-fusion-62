...
export const useStandardizedCreativeCharacters = (
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<StandardizedCharactersResult> => {
      console.log('🚀 Optimized Character Lab fetch - Limit:', limit, 'Offset:', offset, 'Search:', searchQuery);
      
      try {
        // Build super simplified query selecting only essential card fields
        let query = supabase
          .from('characters')
          .select(`
            character_id,
        ...
        // Apply user/public filter with simplified logic
        if (user) {
    ...
    gcTime: 1000 * 60 * 30, // 30 minutes cache - longer retention
